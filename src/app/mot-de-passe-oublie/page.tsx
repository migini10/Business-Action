...
function ForgotPasswordForm() {
  ...
  useEffect(() => {
    ...
    if (urlStep === '3') {
      setStep(3);
      // Clean up the URL
      window.history.replaceState({}, '', '/mot-de-passe-oublie');
    }
  ...
  const handleRequestReset = async (e: React.FormEvent) => {
    ...
    const res: any = await requestPasswordReset(phone, method);
    if (res.success) {
      setSuccessMsg(res.message || null);
      if (method === 'EMAIL') {
        setStep(2);
      } else {
        // Pour WhatsApp, on affiche juste le message de succès sur l'étape 1
        setSuccessMsg("Si un compte correspondant existe, les instructions de récupération ont été envoyées via WhatsApp.");
      }
    } else {
      // Pour le mode BLOCKED lors du test meta si non approuvé
    ...
    <div className="card animate-fade-in" style={{ width: '100%', maxWidth: '450px', padding: '2.5rem 1.5rem', borderRadius: 'var(--radius-xl)', backgroundColor: '#fff', margin: '0 auto', boxShadow: 'var(--shadow-lg)' }}>
      ...
      <div style={{ textAlign: 'center', marginBottom: '1.5rem' }}>
        ...
        <p style={{ color: 'var(--color-text-muted)', margin: 0, fontSize: '0.95rem' }}>
          {step === 1 && "Entrez votre numéro pour recevoir un lien ou code"}
        ...
        <div style={{ backgroundColor: '#F0FDF4', border: '1px solid #BBF7D0', color: '#166534', padding: '1.25rem', borderRadius: 'var(--radius-lg)', marginBottom: '2rem' }}>
          ...
          <div style={{ marginTop: '1rem', paddingTop: '1rem', borderTop: '1px solid #BBF7D0', textAlign: 'center' }}>
            <button type="button" onClick={() => { setStep(1); setMethod('WHATSAPP'); }} style={{ background: 'none', border: 'none', color: '#166534', fontSize: '0.875rem', fontWeight: 600, cursor: 'pointer', textDecoration: 'underline' }}>
              Vous n'avez pas accès à votre email ? Essayer via WhatsApp.
            </button>
          </div>
        </div>
      )}
      ...
      {step === 1 && (
        <form onSubmit={handleRequestReset}>
          <div style={{ marginBottom: '1.5rem' }}>
            <label style={{ display: 'block', fontSize: '0.875rem', fontWeight: 600, color: 'var(--color-text-main)', marginBottom: '0.5rem' }}>Numéro de téléphone</label>
            <PhoneInput
              required
          ...
          <button
            ...
            style={{ width: '100%', padding: '1rem', fontSize: '1rem', borderRadius: 'var(--radius-lg)', opacity: isLoading ? 0.7 : 1, cursor: isLoading ? 'not-allowed' : 'pointer', marginBottom: '1rem' }}
          >
            {isLoading ? 'Envoi en cours...' : (method === 'EMAIL' ? 'Recevoir le code par email' : 'Recevoir un lien de récupération via WhatsApp')}
          </button>
          
          <div style={{ textAlign: 'center' }}>
            {method === 'EMAIL' ? (
              <button type="button" onClick={() => setMethod('WHATSAPP')} style={{ background: 'none', border: 'none', color: 'var(--color-primary)', fontSize: '0.875rem', fontWeight: 600, cursor: 'pointer', textDecoration: 'underline' }}>
                Autres options / Je n'ai pas accès à mon email
              </button>
            ) : (
              <button type="button" onClick={() => setMethod('EMAIL')} style={{ background: 'none', border: 'none', color: 'var(--color-primary)', fontSize: '0.875rem', fontWeight: 600, cursor: 'pointer', textDecoration: 'underline' }}>
                Retour à l'envoi par email (recommandé)
              </button>
            )}
          </div>
        </form>
